...
[Viper](https://github.com/BoxBy/Viper/tree/develop)의 orchestrate harness를 Claude Code 플러그인 시스템에 이식.
...
**Step 3: 그냥 Claude Code 사용**

```
댓글 모듈 페이지네이션 버그 수정해줘
```

끝. Advisor가 작업 난이도를 자동으로 판단해서 알아서 오케스트레이션한다 — 특별한 명령어 필요 없음.

### 어떻게 동작하나요?

| 입력 | 동작 |
|---|---|
| "이 오타 고쳐줘" (trivial) | Advisor가 직접 처리 → 완료 |
| "로그인 API 추가해줘" (standard) | Advisor → `/viper-team --mode=feature-small` (coder + reviewer) |
| "인증 모듈 리팩토링해줘" (complex) | Advisor → `/viper-team --mode=refactor` (architect + coder + reviewer) |
| "결제 시스템 설계해줘" (architecture) | Advisor → `/viper-team --mode=architecture` (5명 워커) |

모든 코드 변경은 자동으로 교차 검증(Pi + Codex 가용 시)되며, Advisor는 non-trivial 작업에 4-step thinking을 적용한다. 이 모든 걸 몰라도 된다 — 원하는 걸 설명만 하면 된다.

### 명시적 명령어 (선택사항)

파워 유저는 자동 라우팅을 우회할 수 있다:

```
/viper-team '댓글 모듈 REST API — auth, CRUD, moderation'
